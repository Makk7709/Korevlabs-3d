...
function Primitive({
  ...
  return (
    <mesh
      position={position}
      rotation={rotation}
      scale={object.kind === "field" ? scale.map((value) => value * pulse) as [number, number, number] : scale}
    ...
    >
      ...
      <meshStandardMaterial
        color={color}
        emissive={selected ? "#8f6700" : "#000000"}
...
export default function SpatialScene({
  ...
  const individualObjects = objects.filter(
    (object) => object.kind !== "algorithm" && object.kind !== "section",
  );
  return (
    <Canvas
      camera={{ position: [6.5, 5.3, 7.5], fov: 43 }}
      dpr={[1, 1.75]}
    ...
    >
      ...
      {physicsMode ? (
        <PhysicsLab parameters={physicsParameters} onTelemetry={onTelemetry} />
      ) : (
        <>
